...
exports.toggleSavedProvider = async (req, res) => {
  try {
    ...
    // Validate that the target provider exists and is a provider
...
// @desc  Get the logged-in customer's saved providers, filtered to only those
//        who currently have at least one approved, active service.
...
exports.getSavedProviders = async (req, res) => {
  try {
    ...
    // Filter saved providers to those who have at least one approved service
    const savedWithServices = await Promise.all(
      customer.savedProviders.map(async (provider) => {
        const hasApprovedService = await Service.exists({
          provider: provider._id,
          approvalStatus: "approved",
          isActive: true,
        });
        return hasApprovedService ? provider : null;
      })
    );

    const filtered = savedWithServices.filter(Boolean);
    return res.status(200).json({ savedProviders: filtered });
  } catch (err) {
