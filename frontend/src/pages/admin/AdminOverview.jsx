...
export default function AdminOverview() {
  ...
  if (error)
    return (
      <div className="mt-4 rounded-[7px] border border-red-200 bg-red-50 px-[0.8rem] py-[0.6rem] text-[0.9rem] text-red-600">
        {error}
      </div>
    );
  if (!stats)
    return <p className="mt-0 text-[0.95rem] text-slate-500">Loading analytics…</p>;
  ...
  return (
    <>
      <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4">
        {cards.map((c) => (
          <div
            className="flex flex-col rounded-lg border border-slate-200 bg-white px-[1.2rem] py-4"
            key={c.label}
          >
            <span className="text-[1.8rem] font-extrabold text-slate-800">{c.value}</span>
            <span className="text-[0.82rem] text-slate-500">{c.label}</span>
          </div>
        ))}
      </div>
